...
async function setupTrash() {
  ...
  const stringifedAmount = trashFormatter.format(amountLeftToRaise)
  const trashAmount = {
    xxl: {
      amount: parseInt(`${stringifedAmount[0]}${stringifedAmount[1]}`),
      icon: "bag"
    },
    xl: {
      amount: parseInt(`${stringifedAmount[2]}`),
      icon: "takeout"
    },
    lg: {
      amount: parseInt(`${stringifedAmount[3]}`),
      icon: "headphones"
    },
    md: {
      amount: parseInt(`${stringifedAmount[4]}`),
      icon: "phone"
    },
    sm: {
      amount: parseInt(`${stringifedAmount[5]}`),
      icon: "toy-car"
    },
    xs: {
      amount: parseInt(`${stringifedAmount[6]}`),
      icon: "bottle"
    },   
  }
